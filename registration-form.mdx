...
const isValidIdentityNumber = (value) => /^[0-9A-Za-z-]+$/.test(value);
...
const isValidIpWhitelist = (value) => {
const ips = value
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
...
showError("Identification / Business registration number is invalid.", 1, "identityNumber");
return;
}

if (!phsCustCode) {
showError("Depository number / Client code at PHS is required.", 1, "phsCustCode");
return;
}
...
if (!technicalContactName) {
showError("Technical contact / title is required.", 1, "technicalContactName");
return;
}

if (!(isValidEmail(technicalContactInfo) || isValidPhone(technicalContactInfo))) {
showError("Email/phone of technical contact must be a valid email (with @) or a digits-only phone number.", 1, "technicalContactInfo");
return;
}
...
// STEP 3 validation
if (!isValidIpWhitelist(ipWhitelist)) {
showError(
  "Registered WAN IP / IP whitelist must be valid IPv4, separated by commas if multiple.",
  3,
  "ipWhitelist"
);
return;
...
if (!appName) {
showError("Name of API-integrated system/application is required.", 3, "appName");
return;
}

if (!techStack) {
showError("Main technical language/platform is required.", 3, "techStack");
return;
}

if (!keySecurityMechanism) {
showError(
  "Storage and protection mechanism for API Key/Secret Key is required.",
  3,
  "keySecurityMechanism"
);
return;
}

if (!rateLimitMechanism) {
showError(
  "Client’s internal kill switch/rate limit mechanism is required.",
  3,
  "rateLimitMechanism"
);
return;
...
if (!testingPlan) {
showError("Testing plan before go-live is required.", 3, "testingPlan");
return;
}

if (!isNotPastDate(goLiveDate)) {
showError("Expected production go-live date must not be in the past.", 3, "goLiveDate");
return;
}

if (devUnit === "third_party" && !thirdPartyName) {
showError("Please enter the third-party entity name.", 3, "thirdPartyName");
return;
}
...
width: "100%",
  ...
  <div id="step-1" style={{ display: "block" }}>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2" style={{ gap: "16px", alignItems: "end" }}>
      ...
      fontSize: "14px",
        ...
        <input
          ...
          required
          pattern="[0-9A-Za-z-]+"
          title="Only letters, numbers and hyphen are allowed."
          style={{
      width: "100%",
      padding: "8px",
      borderRadius: "6px",
      ...
      fontSize: "14px",
        ...
        >
          Depository number / Client code at PHS
        </label>
    ...
    const showError = (message, fieldName) => {
      ...
      if (fieldName) {
        setTimeout(() => form[fieldName]?.focus?.(), 100);
      }
    };
    ...
    if (!isValidIdentityNumber(identityNumber)) {
      showError("Identification / Business registration number is invalid.", "identityNumber");
      return;
    }
    if (!phsCustCode) {
      showError("Depository number / Client code at PHS is required.", "phsCustCode");
      return;
    }
    if (!accountBasic) {
    ...
    if (!technicalContactName) {
      showError("Technical contact / title is required.", "technicalContactName");
      return;
    }
    if (!(isValidEmail(technicalContactInfo) || isValidPhone(technicalContactInfo))) {
      showError("Email/phone of technical contact must be a valid email (with @) or a digits-only phone number.", "technicalContactInfo");
      return;
    }
  ...
  <div id="step-3" style={{ display: "none" }}>
    ...
    if (!isValidIpWhitelist(ipWhitelist)) {
      showError("Registered WAN IP / IP whitelist must be valid IPv4 addresses (e.g. 1.2.3.4), separated by commas if multiple.", "ipWhitelist");
      return;
    }
    if (!appName) {
      showError("Name of API-integrated system/application is required.", "appName");
      return;
    }
    if (!techStack) {
      showError("Main technical language/platform is required.", "techStack");
      return;
    }
    if (!keySecurityMechanism) {
      showError("Storage and protection mechanism for API Key/Secret Key is required.", "keySecurityMechanism");
      return;
    }
    if (!rateLimitMechanism) {
      showError("Client’s internal kill switch/rate limit mechanism is required.", "rateLimitMechanism");
      return;
    }
    if (!testingPlan) {
      showError("Testing plan before go-live is required.", "testingPlan");
      return;
    }
    if (!isNotPastDate(goLiveDate)) {
      showError("Expected production go-live date must not be in the past.", "goLiveDate");
      return;
    }
    if (devUnit === "third_party" && !thirdPartyName) {
      showError("Please enter the third-party entity name.", "thirdPartyName");
      return;
    }
